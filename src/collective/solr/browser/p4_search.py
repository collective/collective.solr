# -*- coding: utf-8 -*-
# This is a literal copy of Products.CMFPlone.browser.search.py (5.0.5)
# for Plone 4.3.x compatibility
# remove if Plone 4.3.x is no longer supported
...
class Search(BrowserView):
    ...
    def filter_query(self, query):
        ...
        if not text:
            ...
            valid = set(valid_indexes).intersection(list(request.form.keys())) or set(
                valid_indexes
            ).intersection(list(query.keys()))
            if not valid:
                return
    ...
    def sort_options(self):
        ...
        return (
            ...
            SortOption(
                self.request, _pmf(u"date (newest first)"), "Date", reverse=True
            ),
