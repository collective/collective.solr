...
class FacettingHelperTest(TestCase):
    ...
    def testFacetDependencies(self):
        ...
        request = {}
        view = DummyView(context, request)
        self.assertEqual(facetParameters(view), (["foo:bar"], dict(foo=["bar"])))
        # overridden on the context
        context.facet_fields = ["bar:foo"]
        self.assertEqual(facetParameters(view), (["bar:foo"], dict(bar=["foo"])))
        # and via the request
    ...
    def testFacetLinks(self):
        ...
        self.assertEqual(
            params(counts[0]["query"]), ["foo=bar", 'fq=portal_type:"Document"']
        )
        self.assertEqual(counts[1]["name"], "Event")
        self.assertEqual(
            params(counts[1]["query"]), ["foo=bar", 'fq=portal_type:"Event"']
        )
        self.assertEqual(counts[2]["name"], "Folder")
        self.assertEqual(
            params(counts[2]["query"]), ["foo=bar", 'fq=portal_type:"Folder"']
        )
    ...
    def testFacetLinksWithMultipleFacets(self):
        ...
        fields = dict(
            foo=dict(Document=10, Folder=3, Event=5), bar=dict(private=2, published=4)
        )
        view = DummyView(context=context, request=request)
        info = convertFacets(fields, view)
        ...
        params = lambda query: sorted(map(unquote, query.split("&")))
        self.assertEqual(
            params(bars[0]["query"]), ["facet.field=foo", 'fq=bar:"published"']
        )
        self.assertEqual(
            params(bars[1]["query"]), ["facet.field=foo", 'fq=bar:"private"']
        )
        # and also the one for 'foo'
        foos = info[0]["counts"]
        self.assertEqual(len(foos), 3)
        self.assertEqual(
            params(foos[0]["query"]), ["facet.field=bar", 'fq=foo:"Document"']
        )
        self.assertEqual(
            params(foos[1]["query"]), ["facet.field=bar", 'fq=foo:"Event"']
        )
        self.assertEqual(
            params(foos[2]["query"]), ["facet.field=bar", 'fq=foo:"Folder"']
        )
    ...
    def testFacetLinksWithMultipleSelectedFacets(self):
        ...
        params = lambda query: sorted(map(unquote, query.split("&")))
        self.assertEqual(
            params(counts[0]["query"]), ["fq=bar:private", 'fq=foo:"Document"']
        )
        self.assertEqual(
            params(counts[1]["query"]), ["fq=bar:private", 'fq=foo:"Folder"']
        )
    ...
    def testSelectedFacetValues(self):
        ...
        self.assertEqual(
            info(),
            [("foo", "Title of X"), ("bar", "Title of Y"), ("bah", "Title of Z")],
        )
